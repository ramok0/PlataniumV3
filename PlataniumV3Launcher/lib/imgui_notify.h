...
#define NOTIFY_TOAST_FLAGS				ImGuiWindowFlags_AlwaysAutoResize | ImGuiWindowFlags_NoDecoration | ImGuiWindowFlags_NoNav
...
public:
	...
	NOTIFY_INLINE auto is_infinite() -> bool {
	...
	NOTIFY_INLINE auto close() -> void {
		this->infinite = false;
		this->dismiss_time = 200;
	...
	{
		if (this->infinite) return ImGuiToastPhase_Wait;
...
public:
	...
	ImGuiToast(ImGuiToastType type, int dismiss_time = NOTIFY_DEFAULT_DISMISS)
	{
		...
		this->infinite = this->dismiss_time == 0;
	...
	ImGuiToast(ImGuiToastType type, int dismiss_time, const char* format, ...) : ImGuiToast(type, dismiss_time) { NOTIFY_FORMAT(this->set_content, format); this->infinite = this->dismiss_time == 0;}
};
...
namespace ImGui
{
	...
	NOTIFY_INLINE VOID MergeIconsWithLatestFont(float font_size, bool FontDataOwnedByAtlas = false)
	{
		...
		GetIO().Fonts->AddFontFromMemoryCompressedTTF((void*)fontawesomefont_compressed_data, fontawesomefont_compressed_size, font_size, &icons_config, icons_ranges);
	}
}
